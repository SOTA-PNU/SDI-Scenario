...
이 서버(A100)는 headless라 모니터가 없습니다. SSH로 쓰는 원격 사용자가 화면을 보는 방법은 세 가지입니다.
...
하네스(`levels/common/carter_env.py`)에 opt-in 녹화 훅이 있습니다. 환경변수만 켜면
런 중 뷰포트가 PNG로 저장됩니다 (러너 코드 수정 불필요):
...
노이즈가 낍니다. `CARTER_RECORD_SIZE=1920x1080` 으로 크게 렌더한 뒤 ffmpeg에서 1280으로
**다운스케일(슈퍼샘플링)** 하면 노이즈가 평균화돼 원본 자체가 깨끗해지고 압축 효율도 좋아집니다:
...
`tmix=frames=5`(5프레임 시간축 평균)가 핵심입니다: 카메라가 고정이라 정지 영역의 스펙클이
프레임 평균으로 거의 사라집니다 (움직이는 로봇에는 자연스러운 모션블러가 생기는 정도).
A/B 실측: atadenoise 단독보다 tmix+hqdn3d 조합이 확연히 깨끗했습니다.
...
> denoise(`hqdn3d`)가 중요합니다: A100은 RT 코어가 없어 렌더에 스펙클 노이즈가 끼고,
> 이게 x264 압축률을 크게 망칩니다 (같은 화질 기준 5배 이상 커짐).
...
⚠ **녹화 런은 시각화 전용입니다.** 캡처 오버헤드로 타이밍이 1프레임쯤 밀립니다
(실측 예: L2 solution ttg 15.250 → 15.233 s). 문서화된 벤치마크 결과
(`results/*_result.json`/CSV)를 녹화 런이 덮어썼다면 `git checkout -- <파일>`로 복원하십시오.
프레임 폴더(`frames_*/`, GB 단위)와 mp4는 `.gitignore`로 커밋에서 제외되어 있습니다.

## 2. WebRTC 라이브 스트리밍 (⛔ 이 서버에서는 불가 — NVENC 없음)

> **실측 결론 (2026-08-04)**: A100은 NVENC(하드웨어 비디오 인코더)가 **0개**인 컴퓨트 전용
> 카드입니다. NVIDIA 스트리밍 스택(NVST)은 NVENC 필수라서, 시그널링(TCP 49100)과 UDP 47998이
> **모두 뚫린 상태를 확인한 뒤에도** 세션이 생성 즉시 `NVST_CCE_DISCONNECTED`로 죽습니다
> (클라이언트 1.0.6/1.1.5 동일). 방화벽·클라이언트 문제가 아닙니다. 라이브는 §3 MJPEG를 쓰십시오.
> 아래 내용은 **NVENC가 있는 호스트**(예: RTX 계열 미러 서버)에서 이 레포를 쓸 때만 유효합니다.
...
`CARTER_LIVE_WAIT=60`: 부팅 완료 후 미션 시작 전에 60초(벽시계) 동안 로봇이 대기합니다 —
이 사이에 클라이언트로 접속하면 주행 시작부터 볼 수 있습니다 (판정에는 영향 없음).

부팅 로그에 `[env] WebRTC livestream enabled` 가 뜨고 서버가 **TCP 49100**(시그널링)으로
리스닝합니다. 시청은 NVIDIA **"Isaac Sim WebRTC Streaming Client"** (Windows/Linux/macOS,
NVIDIA 사이트에서 배포)에 서버 IP를 넣고 접속.

**서버 방화벽 (이 호스트에서 실측 확인된 함정):** 이 서버는 ufw가 기본 **DROP** 정책으로
켜져 있어, 스트림이 떠 있어도 클라이언트에는 아무것도 안 뜹니다. 1회만 열어주면 됩니다:
...
등)이 문서 지정 버전입니다. 최신 1.1.5는 Isaac 5.x용이라 4.5 서버와 시그널링이 안 맞을 수 있습니다 —
안 붙으면 1.0.6으로 시도.
...
- 이 서버의 실제 IP는 사설망 `10.254.182.72` 이고, 외부에서는 `164.125.19.138:10022`
  SSH 포워딩으로만 들어옵니다. 즉 **노트북이 같은 내부망(10.254.x.x 대역)에 있거나**,
  관리자가 두 포트를 외부로 포워딩해 줘야 라이브 시청이 됩니다.
- SSH 터널(`ssh -L 49100:...`)로는 TCP 시그널링만 넘어가고 **UDP 미디어는 터널을 못
  넘습니다** → 접속은 되는데 화면이 안 나오는 형태로 실패합니다.
...
NVENC 없이 CPU만으로 동작합니다: 하네스가 뷰포트를 JPEG 링버퍼로 `/dev/shm`에 캡처하고,
경량 HTTP 서버(`levels/common/mjpeg_server.py`, stdlib만 사용)가 이를
multipart MJPEG로 스트리밍합니다. 노트북에서는 **브라우저로 열기만 하면 됩니다** (앱 설치 불필요).
TCP 한 포트만 쓰므로 SSH 터널로도 동작합니다 (WebRTC와 달리 UDP가 없음).
...
서버 기동·환경변수·종료 후 벤치 결과 원복까지 알아서 처리합니다. 아래는 수동 버전입니다.
