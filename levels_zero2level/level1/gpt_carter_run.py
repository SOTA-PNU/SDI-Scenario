...
def controller(t, pose, env):
    """Simple proportional controller for level‑1.

    * Rotate towards the target yaw first.
    * Then drive straight forward along the y‑axis.
    * Stop when position and yaw are within the tolerances defined in the
      mission table.
    """
    x, y, yaw = pose
    goal_x, goal_y, goal_yaw = GOAL

    # Tolerances from the mission definition
    pos_tol = M["position_tolerance_m"]
    yaw_tol = M["yaw_tolerance_rad"]

    # Compute errors
    yaw_error = _normalize_angle(goal_yaw - yaw)
    dx = goal_x - x
    dy = goal_y - y
    dist = math.hypot(dx, dy)

    # Check if we have reached the goal within tolerances
    if dist <= pos_tol and abs(yaw_error) <= yaw_tol:
        return 0.0, 0.0, True

    # If orientation is not good enough, prioritize turning
    if abs(yaw_error) > 0.1:
        # Simple proportional turn, limited to ±1.0 rad/s
        w = max(-1.0, min(1.0, 1.5 * yaw_error))
        v = 0.0
        return v, w, False

    # Orientation is acceptable – drive forward toward the goal
    # Use a constant forward speed; the harness will stop on collision.
    v = 0.6  # m/s, modest speed for safe navigation
    w = 0.0
    return v, w, False
