...
미션(레벨 1): 로봇은 (-6.0, -1.0)에서 yaw=π(-x 방향)를 바라보고 스폰된다. 목표
(-6.0, 5.0)은 **+y 방향으로 6.0 m** — 스폰 헤딩과 다른 방향이므로 **회전 제어가 필요**하다.
통로는 끝까지 비어 있다. 목표에 도달해 **도착 자세 yaw=+1.5708(+y)** 로 정렬한 뒤 정지하라.
가는 동안 어떤 것에도 닿으면 안 된다.

성공 조건: 최종 위치가 목표에서 0.75 m 이내 **그리고** |yaw 오차| ≤ 0.26 rad,
충돌 0회, 제한 시간 120 s.

접근 지침: 3단계 포인트 컨트롤러가 정석이다 — (1) 목표 방위각(`atan2`)과 현재 yaw 의
오차를 [-π, π]로 래핑해 비례(P) 제어로 회전하되, 오차가 크게 벌어져 있는 동안은 전진하지
말고 제자리에서 돌아라; (2) 방향이 맞으면 순항 속도로 주행하며 방위 오차를 계속 보정하고,
목표에 가까워지면 감속하라; (3) 목표 반경 안에 들어오면 제자리 회전으로 도착 yaw 를 맞춘
뒤 `done=True`. 각속도에는 상한을 두어라.
