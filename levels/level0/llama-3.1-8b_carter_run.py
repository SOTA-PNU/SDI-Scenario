"""LEVEL 0 mission runner - forward-only reach.

MISSION (scenarios/nova_carter_warehouse_level0.yaml):
  The Nova Carter spawns at world (-6.0, -1.0) facing yaw=pi (-x direction).
  The goal (-8.0, -1.0) lies 2.0 m STRAIGHT AHEAD of that heading in a free
  aisle (pallets block the aisle from ~2.5 m).  Reach it using forward-only
  motion (v >= 0, w == 0) and stop.

PASS CRITERIA (evaluated by the fixed harness, not by this file):
  reached_goal   final distance to goal <= 1.0 m   (yaw not scored at L0)
  timeout_s      60 s of sim time
  (no_collision is not scored at L0 - the aisle is empty)
...
ROBOT API available to the controller:
  ...
  return (v, w, done):
    ...
    env   harness handle: env.raycast_scan(n_beams, fov_deg, z, max_range)
          -> [(bearing_rel_to_heading, distance_m), ...]  (not needed at L0)
...
def controller(t, pose, env):
    ...
    target_distance = ((pose[0] - (-8.0))**2 + (pose[1] - (-1.0))**2)**0.5

    # If the robot is close enough to the target, stop moving
    if target_distance <= 1.0:
        return 0, 0, True

    # Otherwise, move forward at a constant speed
    return 1.0, 0, False
