...
pose 만 보고 조향합니다 — 센서가 없으니 lane 위에 나타난 박스를 알 길이 없습니다.
...
의미: L1 솔루션의 결함이 아니라 **L1 스킬셋의 한계**를 정확히 노출합니다. yaw 오차 0.000
이 보여주듯 주행·정렬은 완벽한데, "보는 능력"이 없어서 실패합니다. 또한 충돌 파트너가
정확히 `/World/debug_obstacle` 로 잡혀 **장애물 주입 + contact oracle 경로가 이 레벨에서도
검증**됐습니다 (L1 base 의 카드박스 충돌에 이은 두 번째 양성 대조).

## 3. 풀이 과정 — 1차 시도(gap-follow)의 실패와 진단

정답에 이르기까지의 실측 루프를 그대로 기록합니다 (`attempt1_gapfollow_*.{json,csv}` 보존).
...
궤적이 말해 주는 것: 회피 자체는 **정상 발동했습니다**. y≈0.39 (박스 앞면까지 1.31 m)에서
열린 동쪽으로 조향을 시작했고, 스캔 기하·조향 부호·발동 거리 모두 의도대로였습니다.
실패 원인은 둘:

1. **코너 여유 부족** — 이웃-레이 각도 마진 ±0.20 rad 은 장애물이 2.2 m 거리일 때
   0.45 m 의 측면 여유라는 가정인데, 박스에 1.0 m 까지 접근한 시점엔 같은 각도가
   **0.2 m** 로 줄어듭니다. 로봇 반폭 수준이라 동쪽 앞모서리 (-5.8, 1.7) 를 스쳤습니다.
2. **복귀가 너무 이름** — 모서리가 전방 콘을 벗어나는 순간 위협이 사라져 L1 동작으로
   복귀, goal 쪽(왼쪽)으로 되꺾었습니다. 섀시는 아직 박스 옆면을 지나는 중이었고
   t=8.617 s, (-5.77, 1.44) 에서 박스에 걸렸습니다.

**재설계**: 두 결함 모두 "마진이 거리와 무관하게 고정"인 데서 왔습니다 → 마진을 거리의
함수로 만드는 **bearing 팽창(VFH-lite)** 으로 교체했습니다. 각 히트 레이 `(b, r)` 가
`b ± atan2(SIDE_MARGIN, r)` 방위 쐐기를 차단합니다: 가까울수록 넓게 막히므로 (1)이
해결되고, 박스 옆을 지나는 동안 측면 히트가 goal 방향을 계속 차단하므로 명시적
상태(state) 없이 (2)의 조기 복귀도 사라집니다.
...
1. **거리-반비례 팽창**: `BLOCK_RANGE`(2.2 m) 안의 모든 히트가 자기 주변 방위를
   ...
   `SIDE_MARGIN`(0.55 m)으로 유지됩니다.
2. **goal 방향이 안 막혔으면 L1 그대로** — 회피 코드는 필요할 때만 개입합니다
   (`r < min(BLOCK_RANGE, d)`: goal 보다 먼 히트는 애초에 장애물이 아님).
3. **막혔으면 가장 goal 쪽의 열린 방위로** 감속 주행. 우회 중엔 박스 옆면 히트가
   goal 방향을 계속 차단하므로, 박스를 실제로 벗어나야만 복귀가 시작됩니다.
...
도착 정렬 후 종료. 1차 시도의 두 실패 원인(코너 여유·조기 복귀)이 정확히 사라졌습니다.
