...
있습니다). 프롬프트는 모델에게 `base_carter_run.py` 를 복사한 새 파일
`<모델 본인 이름>_carter_run.py` 를 만들어 그 파일의 `[EDIT REGION]` 블록만 수정하라고
지시합니다 — base 는 손대지 않으므로 비교 기준이 보존됩니다. `prompted_carter_run.py` 는
그 프롬프트만 보고 작성한 **기준 구현**입니다.

에이전트형 CLI 사용 예 — "levels/level0/PROMPT.md 를 읽고 그대로 수행해줘." 모델이
파일을 만들면 (프롬프트가 실행·채점은 금지합니다) 검증은 채점자가 별도로 합니다:

```bash
# 모델이 만든 파일 검증 (저장소 루트에서)
python3 levels/common/check_edit_region.py levels/levelN/<모델이름>_carter_run.py  # 블록 밖 == base 확인
bash levels/run_isaac.sh levels/levelN/<모델이름>_carter_run.py                    # Isaac 실측
# 판정: levels/levelN/results/<모델이름>_result.json 의 verdict
```

블라인드 주의: 에이전트에게 저장소를 열어 주면 같은 폴더의 정답 파일
(solution/prompted)을 읽을 수 있습니다. 엄격한 평가라면 해당 파일을 치우거나 참고
금지를 입력에 덧붙이십시오.

파일을 만들 수 없는 채팅형 모델은 PROMPT.md 전문 + base 전문을 붙여넣어 블록만 받고
채점기로 이식·실측합니다 (마커·코드펜스가 섞여 있어도 정리해 이식):
