...
통과하는가"** 를 L0 → L3 4단계로 쌓아 올린 CV-Infra 표준 입력 인스턴스입니다.

> **현 상태 (2026-08-03 실측 완료)**: 4개 레벨 전부 Isaac Sim 4.5 (A100, seed 42)에서 **직접
> 실행·검증**됐습니다. `levels/levelN/` 에 base 코드(FAIL 실측)·solution 코드(PASS 실측)·과정 리포트·
> 결과 원본(JSON/CSV/궤적 PNG)이 있습니다 — cv_infra 없이 스탠드얼론 하네스로 돌렸고, 목적은 **NPU
> 로컬 LLM 벤치마크 픽스처**입니다. **직접 돌려보려면 → [`QUICKSTART.md`](QUICKSTART.md)**.
...
> L3의 `max_time_to_goal_s` 는 실측 기반으로 45→12 s 재조정됐습니다(`levels/level3/REPORT.md`).
...
각 레벨은 **바로 아래 레벨의 조건을 전부 포함**하고 하나씩만 더 요구합니다(단조 누적). 그래서
"어느 레벨에서 처음 깨지는가"가 곧 진단 결과입니다.
...
**L3에서만 "도달했는데 실패"(너무 느림)가 가능**합니다. L0~L2는 도달·무접촉이 곧 성공입니다.

레벨 간 차이는 `scenario.goal` · `scenario.debug_obstacle` · `scenario.timeout_s` ·
`acceptance_criteria` **네 곳에만** 있습니다. `interface.adapter_config`는 정본의 실측 fill과 완전히 동일합니다.

## 값의 출처

레벨 설계에 쓴 수치는 전부 cv-infra-user 정본/변형 파일에 기록된 실측치에 앵커했습니다.
...
- L2/L3 장애물 높이 **1.0 m** — 정본 변형 `nova_carter_warehouse_obstacle_fail.yaml`의 0.15 m 저상 박스가
  *nav이 못 보고 긁는* 의도적 fail(run7 collision_count 3603)이었던 것의 **반대 짝**입니다. 두 파일을 같이
  돌리면 "인지 실패"와 "회피 실패"가 분리 진단됩니다.

## 실행 전 확인할 것 (1건)

⚠ yaw 판정은 `goal_orientation_wxyz` 가 설정되어야 활성화됩니다(`scenario.goal.yaw` 만으로는 비활성).
L1/L3 params에 yaw=1.5708의 쿼터니언 `[0.7071068, 0, 0, 0.7071068]`을 넣어뒀는데, criteria params는
미지 키를 **loud-reject 하지 않고 조용히 무시**합니다(정본의 draft `goal_tolerance_m` 사례). 키 이름이
틀리면 L1이 "위치만 판정"으로 **조용히 퇴화**하므로, 첫 런에서 `result.json`의 `reached_goal` detail에
yaw 항목이 실제로 잡히는지 확인하십시오. 미지원이면 `scenarios/max_time_to_goal.py`를 본뜬 yaw 판정
커스텀 oracle이 대안입니다.

## 커스텀 oracle

`scenarios/max_time_to_goal.py`는 cv-infra-user의 플러그인 예시를 **그대로 가져온 사본**입니다(L3의
`oracle: "max_time_to_goal:MaxTimeToGoalOracle"` 참조가 이 저장소만으로 동작하려면 시나리오 YAML과
같은 디렉토리에 있어야 합니다). 원본·최신본은 항상
[cv-infra-user/scenarios/max_time_to_goal.py](https://github.com/yongjunshin/cv-infra-user/blob/main/scenarios/max_time_to_goal.py)를 따릅니다.
...
현재 계약으로는 표현할 수 없습니다 — `scenario.goal` 단일(다중 웨이포인트 불가) · `scenario.debug_obstacle`
